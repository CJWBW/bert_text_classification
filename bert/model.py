...
class BERTClassifier(nn.Module):
    def __init__(self, model_option, n_classes):
        ...
        self.fc1 = nn.Linear(self.bert.config.hidden_size * 2 + 8, 120)
        self.out = nn.Linear(120, n_classes)

    # consider metadata
    def forward(self, input_ids, metadata, attention_mask1, categorical_numerical_data, attention_mask2):
...
class Model:
    ...
    def train_epoch(model, optimizer, scheduler, train_dataloader, n_examples, device, loss_fn):
        ...
        for data in train_dataloader:
            ...
            categorical_numerical_data = data["categorical_numerical_data"].to(device)

            outputs = model(
                input_ids=input_ids,
                metadata=metadata,
                categorical_numerical_data=categorical_numerical_data,
